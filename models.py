...
class wrapped_GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
                 dropout=0.5, save_mem=True, use_bn=True):
        super(wrapped_GCN, self).__init__()
    ...
    def reset_parameters(self):
        ...
        for bn in self.bns:
            bn.reset_parameters()
        self.edge_predictor[0].reset_parameters()
        self.edge_predictor[2].reset_parameters()
    ...
    def forward(self, data):
        ...
        #x = self.convs[-1](x, data.graph['edge_index'])
        N=x.shape[0]
        #print("FORWARD STEP ############ ",x.shape)
        xe1=x.unsqueeze(0).repeat(N,1,1)
        xe2=x.unsqueeze(1).repeat(1,N,1)
        #print("FORWARD STEP EDGES ###### ",xe1.shape,xe2.shape)
        res=torch.cat((xe1,xe2),dim=-1)
        res=self.edge_predictor(res)
        #print("FORWARD STEP RESULT ##### ",res.shape,res.flatten().shape)
        return res.flatten()
