...
def load_nc_dataset(args):
    ...
    print(dataname)
    if dataname == 'atoms':
        dataset = load_atom_dataset('0','364','148',args.edge_threshold)
...
def read_graph_weight(file_path):
    ...
    for row in rows:
        real_numbers = [complex(num.strip()).real for num in row.split(",") if num.strip()]
        real_array.append(real_numbers)
    
    return np.array(real_array, dtype=np.float32)

def load_atom_dataset(file1,file2,file3,edge_threshold=100):
    dataset = NCDataset('atoms')
    structure = Structure.from_file(f'{DATAPATH}/POSCAR-{file1}')
    graph1 = read_graph_weight(f'{DATAPATH}/matrix-{file1}.txt')
    n=len(structure)
    N=n*4
    features=[]
    for x in range(n):
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,1,0,0,0])
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,0,1,0,0])
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,0,0,1,0])
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,0,0,0,1])
    
    structure = Structure.from_file(f'{DATAPATH}/POSCAR-{file2}')
    graph2 = read_graph_weight(f'{DATAPATH}/matrix-{file2}.txt')
    m=len(structure)
    M=m*4
    for x in range(m):
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,1,0,0,0])
        ...
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,0,0,0,1])
    '''
    structure = Structure.from_file(f'{DATAPATH}/POSCAR-{file3}')
    graph3 = read_graph_weight(f'{DATAPATH}/matrix-{file3}.txt')
    k=len(structure)
    K=k*4
    for x in range(k):
        features.append([structure[x].x,structure[x].y,structure[x].z,structure[x].a,structure[x].b,structure[x].c,1,0,0,0])
    ...
    graph=np.block([
        [graph1, np.zeros((N, M+K), dtype=np.float32)],
        [np.zeros((M, N),dtype=np.float32), graph2, np.zeros((M, K), dtype=np.float32)],
        [np.zeros((K, N+M), dtype=np.float32), graph3]
    ])
    '''
    num_nodes=N+M
    graph=np.block([
        [graph1, np.zeros((N, M), dtype=np.float32)],
        [np.zeros((M, N),dtype=np.float32), graph2]
    ])
    ...
    def dist(i,j):
        if ty(i)!=ty(j):
            return 1e9
        return ((features[i][0]-features[j][0])**2+(features[i][1]-features[j][1])**2+(features[i][2]-features[j][2])**2)**0.5
    A=np.zeros((num_nodes,num_nodes),dtype=np.bool)
    for u in range(num_nodes):
        for v in range(num_nodes):
            if dist(u,v)<=edge_threshold:
                A[u][v]=1

    edge_index=torch.tensor(A.nonzero(),dtype=torch.long)
    node_feat=torch.tensor(features,dtype=torch.float)
    label=torch.tensor(graph.flatten(),dtype=torch.float)
    dataset.graph = {'edge_index': edge_index,
                     'edge_feat': None,
                     'node_feat': node_feat,
                     'num_nodes': num_nodes}
    dataset.label = label.squeeze()

    # train val test
    train_mask=torch.zeros(num_nodes*num_nodes,dtype=torch.int32)
    val_mask=torch.zeros(num_nodes*num_nodes,dtype=torch.int32)
    test_mask=torch.zeros(num_nodes*num_nodes,dtype=torch.int32)

    sel=[0]*(M*M)
    tmparr = np.random.choice(M*M, M*M//2, replace=False)
    for x in tmparr:
        sel[x]=1
    for i in range(num_nodes):
        for j in range(num_nodes):
            if i<N and j<N:
                train_mask[i*num_nodes+j]=1
            elif i>=N and j>=N:
                if sel[(i-N)*M+(j-N)]:
                    val_mask[i*num_nodes+j]=1
                else:
                    test_mask[i*num_nodes+j]=1
